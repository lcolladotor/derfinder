...
The following plots are [MA](http://en.wikipedia.org/wiki/MA_plot)-style plots comparing each group vs the first one. Note that the fold change as calculated here is not taking into account the library sizes.

They are only shown when there are two or more groups.

```{r MAstyle, fig.width=10, fig.height=10, dev=device}
for(j in grep("foldChange", colnames(values(fullRegions)))) {
	ma <- data.frame(mean=fullRegions$meanCoverage, foldChange=values(fullRegions)[, j]))
	groups <- strsplit(gsub("foldChange", "", colnames(values(fullRegions))[j]), "vs")[[1]]
	p.ma <- ggplot(ma, aes(x=log2(mean), y=foldChange)) + geom_point() + ylab("Fold Change (log2)") + xlab("Mean coverage (log2)") + labs(title=paste("MA style plot", groups[1], "vs", groups[2])
	print(p.ma)
}
```
...
if(nBestRegions > 0) {
	plotRegionCoverage(regions=fullRegions, regionCoverage=regionCoverage, groupInfo=optionsStats$groupInfo, nearestAnnotation=regions.df, annotatedRegions=fullAnnotatedRegions, N = nBestRegions, colors=NULL, scalefac = 32, ask = FALSE) 
}
```
...
```{r plotGenome, message=FALSE, eval=makeBestClusters, echo=makeBestClusters}
## Select best by area
bestArea <- head(order(fullRegions$area, decreasing=TRUE), nBestClusters)
...
```{r bestArea, message=FALSE, fig.width=11, fig.height=13, dev=device, eval=makeBestClusters, echo=makeBestClusters}
