...
This report is meant to help explore the results of the `derfinder` `r mycitep(bib[["derfinder"]])` package. While the report is rich, it is meant to just start the exploration of the results and exemplify some of the code used to do so. You will most likely need a more in-depth analysis for your specific data set.
...
These plots are only shown when there are two or more groups. A total of `r length(grep("foldChange", colnames(values(fullRegions))))` plot(s) were made.
...
The following plots were made using `ggbio` `r mycitep(bib[["ggbio"]])` which in turn uses `ggplot2` `r mycitep(bib[["ggplot2"]])`. For more details check `plotOverview` in `derfinder` `r mycitep(bib[["derfinder"]])`.
...
Below is an interactive table with the top `r min(nrow(regions.df), 2000)` regions (out of `r nrow(regions.df)`) as ranked by area. This table was generated using `rCharts` `r mycitep(bib[["rCharts"]])`. Inf and -Inf are shown as 1e100 and -1e100 respectively.
...
Note that in `derfinder` `r mycitep(bib[["derfinder"]])` the re-sampling of the samples is done without replacement. This is done in an effort to avoid singular model matrices. While the sample balance is the same across the permutations, what changes are the adjusted variables (including the column medians).
