...
These plots are only shown when there are two or more groups. A total of number of groups - 1 plots are made.
...
for(j in grep("foldChange", colnames(values(fullRegions)))) {
	...
	p.ma <- ggplot(ma, aes(x=log2(mean), y=foldChange)) + geom_point(size=1.5, alpha=1/5) + ylab("Fold Change (log2)\nRed dashed line at mean; blue line is loess fit") + xlab(paste("Mean coverage (log2) using only groups", groups[1], "and", groups[2])) + labs(title=paste("MA style plot group", groups[1], "vs group", groups[2])) + geom_hline(aes(yintercept=foldMean), data=fold.mean, colour="#990000", linetype="dashed") + geom_smooth(aes(y=foldChange, x=log2(mean)), data=subset(ma2, mean > 0), method=loess)
	print(p.ma)
}
```
...
Below are the plots for the top `r nBestRegions` regions as ranked by area.


```{r plotRegions, fig.width=10, fig.height=10, dev=device}
if(nBestRegions > 0) {
	plotRegionCoverage(regions=fullRegions, regionCoverage=regionCoverage, groupInfo=optionsStats$groupInfo, nearestAnnotation=regions.df, annotatedRegions=fullAnnotatedRegions, N = nBestRegions, colors=NULL, scalefac = 32, ask = FALSE, verbose = FALSE) 
}
```
...
## Select clusters from the top nBestClusters regions
clusterChr=paste0(as.integer(head(fullRegions$cluster, nBestClusters)), chr=as.character(seqnames(head(fullRegions, nBestClusters))))
bestArea <- seq_len(nBestClusters)[!duplicated(clusterChr)]
...
`r ifelse(makeBestClusters, paste("Below are the best", nBestClusters, "region clusters ordered by area."), "No plots were generated as requested.")`
 

```{r bestClusters, message=FALSE, fig.width=11, fig.height=13, dev=device, eval=makeBestClusters, echo=makeBestClusters}
