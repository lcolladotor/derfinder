...
* _coverageMatrix_ is the matrix with one row per region and one column per sample. The different matrices for each of the chromosomes can then be merged prior to using `r Biocpkg('limma')` or other packages. Note that the counts are generally not integers, but can easily be rounded if necessary.

The actual ERs are arranged in a `GRanges` object.
...
## Perform DE analysis
system.time( dse <- DESeq(dse, test = 'LRT', reduced = ~ gender, fitType = 'local') )
...
annoRegs <- annotateRegions(ers, genomicState$fullGenome)

library('derfinderPlot')
vennRegions(annoRegs, counts.col = 'blue')
```
