...
#'
#' @return 
#' A list with one element per sample. Then per sample, a list with two vector elements: \code{values} and \code{weights}. The first one is the coverage value and the second one is the number of bases with that value.
#'
...
collapseFullCoverage <- function(fullCov, colsubset=NULL, save=FALSE, verbose=FALSE) {
	...
	if(verbose) message(paste(Sys.time(), "sampleDepth: Sorting fullCov"))
	sortedFull <- lapply(fullCov, function(x) {
		sapply(x, sort)
	})

	## Collapse chrs by sample
	if(verbose) message(paste(Sys.time(), "sampleDepth: Collapsing chromosomes information by sample"))
	...
	if(save) {
		if(verbose) message(paste(Sys.time(), "sampleDepth: Saving collapsedFull"))
