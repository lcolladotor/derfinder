...
calculateStats <- function(coveragePrep, models, mc.cores=getOption("mc.cores", 2L), adjustF=0, lowMemDir=NULL, verbose=TRUE) {
	...
	fstats.output <- mclapply(mclapplyIndex, fstats.apply, data=coverageProcessed, mod=models$mod, mod0=models$mod0, adjustF=adjustF, lowMemDir=lowMemDir, mc.cores=mc.cores)
